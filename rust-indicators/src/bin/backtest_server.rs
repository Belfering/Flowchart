...
fn should_use_vectorized(req: &FullBacktestRequest) -> bool {
    // Parse strategy to check - use custom deserializer for deeply nested strategies
    let mut deserializer = serde_json::Deserializer::from_str(&req.payload);
    deserializer.disable_recursion_limit();
    let result: Result<FlowNode, _> = serde::de::Deserialize::deserialize(&mut deserializer);
